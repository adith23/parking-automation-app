...
const EditParkingLotScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchLotData = async () => {
      try {
        setIsLoading(true);
        const response = await api.get(`/owner/parking-lots/${parkingLotId}`);
        const lot = response.data;

        setName(lot.name);
        setAddress(lot.address);
        if (lot.gps_coordinates) {
          setSelectedLocation({
            latitude: lot.gps_coordinates.latitude,
            longitude: lot.gps_coordinates.longitude,
          });
        }
        setTotalSlots(lot.total_slots);
        setPricePerHour(String(lot.price_per_hour));
        setOpenTime(lot.open_time ? lot.open_time.slice(0, 5) : "");
        setCloseTime(lot.close_time ? lot.close_time.slice(0, 5) : "");
        setAdditionalInfo(lot.additional_info?.rules_and_regulations || "");
        setPhotos(lot.media_urls?.photos || []);
        setVideos(lot.media_urls?.videos || []);
      } catch (error) {
        console.error("Failed to fetch parking lot data:", error);
        Alert.alert("Error", "Could not load parking lot data.", [
  ...
  const handleUpdate = async () => {
    ...
    try {
      await api.put(`/owner/parking-lots/${parkingLotId}`, payload);
      Alert.alert("Success", "Parking lot has been updated successfully!", [
        { text: "OK", onPress: () => router.back() },
      ]);
    } catch (error) {
      const errorMessage =
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          <Text style={{ marginTop: 10 }}>Loading Lot Data...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Update Parking Lot</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        >
          ...
          <Text style={styles.submitButtonText}>Map Parking Slots</Text>
        </TouchableOpacity>
        ...
        >
          {submitting ? (
            ...
            <Text style={styles.submitButtonText}>Update Parking Lot</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
...
export default EditParkingLotScreen;
